...
import type {MatrixResult} from "$lib/types/MatrixResult";
import {PeopleComparator} from "$lib/logic/PeopleComparator";
 ...
 */
export const buildTeams = (persons: Array<Person>, partialOptions?: Partial<TeamBuilderOptions>): [Array<Team>, Array<Person>, Array<Array<number>>] => {
    ...
    return [
        bestTeams.map(t => ({persons: t, score: computeTeamScore(context, ...t)})).sort((t1, t2) => t2.score - t1.score),
        bestRemaining,
        context.matrixScore
    ];
};
...
const buildTeamsInternalIteration = (
    persons: Array<Person>, centroids: Array<Person>, context: Context
    ...
    for(;;) {
        ...
        remaining.splice(remaining.findIndex(r => r === bestMatch), 1);
        team.push(bestMatch);
...
const areTeamsComplete = (teams: Array<Array<Person>>, remaining: Array<Person>, context: Context) => {
...
const computePairScoreMatrix = (matrixResults: Array<Array<MatrixResult>>): Array<Array<number>> => {
