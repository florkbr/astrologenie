...
export class AspectCalculator {
    ...
    */
     ...
     */
    private arePlacementsInAspect(aspect: Aspect, firstPlacement: Placement, secondPlacement: Placement): boolean {
        ...
        const largetPlacement: number = Math.max(firstPlacement.getDegrees(), secondPlacement.getDegrees());
        const smallerPlacement: number = Math.min(firstPlacement.getDegrees(), secondPlacement.getDegrees());

        // Firstly check regularly and then in case the aspect is reversed.
        if ((largetPlacement >= (smallerPlacement + lowerLimit)) && (largetPlacement <= (smallerPlacement + upperLimit))) {
            return true;
        } else if ((largetPlacement >= (smallerPlacement + (360 - aspectDegree) - allowedOrb))
     ...
     */
    private calculateTotalOrb(aspect: Aspect, firstPlacement: Placement, secondPlacement: Placement): number {
        const largerDegree: number = Math.max(firstPlacement.getDegrees(), secondPlacement.getDegrees());
        const smallerDegree: number = Math.min(firstPlacement.getDegrees(), secondPlacement.getDegrees());
     ...
     */
    private calculateStat(aspect: Aspect, orb: number, firstElement: Element, secondElement: Element): void {
    ...
    */
     ...
     */
    private calculateStatNature(aspect: Aspect, firstElement: Element, secondElement: Element): StatNature {
        switch (aspect) {
